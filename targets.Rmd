...
Load in the raw data files from Google Drive and the OSF. Most raw data files will have three targets associated with them - one for their ID on Google Drive, one for the modification date of the file, and one that actually represents the data loaded into R. The modification date targets are set to run every time `tar_make()` is called so that the file is pulled again if it is ever updated on Google Drive.

### Paper- and Journal-level Data

```{targets raw-paper}
list(
  # COS SCORE Data Google Drive - raw/paper_level ----
...
```{targets raw-claims}
list(
  # COS SCORE Data Google Drive - raw/claim_extraction ----
...
```{targets helpers}
list(
  # COS SCORE Data Google Drive - raw/rr_variables ----
  ...
  tar_target(repli_cases_projects_gsheet,
             "1y57RNOH96x8dXYc8gxBbCQOmuaMVOoaD0KgJYfoITmg"),
  tar_target(repli_cases_projects_moddate,
             get_google_mod_date(repli_cases_projects_gsheet),
             cue = tar_cue("always")),
  tar_target(repli_cases_projects,
             read_google_sheet(repli_cases_projects_gsheet,
                               repli_cases_projects_moddate,
                               sheet = "rr_projects")),
...
list(
  # COS SCORE Data Google Drive - raw/original_variables ----
  ...
  tar_target(
    ...
    read_google_tsv(
      ...
      col_types = list(
        ...
        # There is a weird data corruption in this field, but it gets fixed
        # in the changelog:
  ...
  # SCORE-Phase 1 RR and original article analyses (https://osf.io/3fsbm/) ----
  ...
  # SCORE Google Drive ----
  # (https://drive.google.com/drive/folders/0AHots-bNz56yUk9PVA)
...
list(
  ...
  # SCORE Replication cases ----
  # https://docs.google.com/spreadsheets/d/1y57RNOH96x8dXYc8gxBbCQOmuaMVOoaD0KgJYfoITmg/edit#gid=0
  tar_target(replication_cases_gsheet,
             "1y57RNOH96x8dXYc8gxBbCQOmuaMVOoaD0KgJYfoITmg"),
  tar_target(replication_cases_moddate,
             get_google_mod_date(replication_cases_gsheet),
             cue = tar_cue("always")),
  tar_target(replication_cases,
             read_google_sheet(replication_cases_gsheet,
                               replication_cases_moddate,
                               sheet = "repli_export"))
...
list(
  ...
  tar_target(repro_export,
             create_repro_export(reproduction_qa,
