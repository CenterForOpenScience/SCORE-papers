calculate_repro_summary <- function(orig_dataset,
                                    repro_export,
  ...
  repro_secondary <- rr_confrontations_repro_claim %>% 
    mutate(
      unique_claim_id = str_c(paper_id, "_", confrontation_claim4_id),
      repro_secondary_criteria = confrontation_repro_secondary_criteria_num > 0
    ) %>% 
    select(unique_claim_id, rr_id, repro_secondary_criteria)
  ...
  repro_added <- outcomes_thresholds %>%
    ...
    left_join(repro_secondary, by = c("unique_claim_id", "rr_id")) %>% 
