transform_paper_metadata <- function(paper_metadata_raw, score_paper_status) {
  ...
  paper_metadata_raw %>%
    ...
    left_join(score_paper_status %>% 
                select(paper_id, p1_delivery, p2_delivery),
              by = "paper_id") %>%
    ...
    # into R
    rows_update(
      tibble(paper_id = "q3XN", author_last = "Na"),
      by = "paper_id"
    )
}
