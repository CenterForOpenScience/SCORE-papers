# Convenience function for finding tags in the 
...
paper_5_stats <- function(iters = 100,repli_outcomes,orig_outcomes,df.papers=NA){
  ...
  # Stats
  {
    ...
    p_effect_size_ratio_v_orig <- paste0(round(median_SER_effect_size_ratio_v_orig$point.estimate,3)*100,
                                         "% (95% CI ",
    ...
    repli_score_criteria_met <- bootstrap.clust(data=repli_outcomes[c("paper_id","claim_id","repli_score_criteria_met")],FUN=
                                                  function(data) {
                                                    data <- data %>% add_count(paper_id)
                                                    data$weight <- 1/data$n
