...
placeholder_stats <- function(iters=1000){
  ...
  # Data preparation
  {
    ...
    # Merge in paper metadata
      ...
      {
        repro_outcomes_expanded <- paper_status_tracking %>%
          filter(RR) %>%
  ...
  # Stats
  {
    # Table 2
    {
      ...
      # Papers with claims
      r1 <- format.row(paper_status_tracking %>% filter(p1_delivery))
      # Papers eligible for reproduction
      r2 <- format.row(paper_status_tracking %>% filter(RR))
      # Papers with multiple claims
      r3 <- format.row(paper_status_tracking %>% filter(bushel))
      # Papers with single claim
      r4 <- format.row(paper_status_tracking %>% filter(RR & !bushel))
      # Papers with data available
      r5 <- format.row(rr_internal_tracking %>%
                         semi_join(paper_status_tracking %>% filter(RR), by = "paper_id") %>%
      ...
      r6 <- format.row(rr_internal_tracking %>%
        filter(field != "covid") %>%
    ...
    # Results: Process reproducibility
    {
      ...
      n_papers_restricted_not_avail <- pr_outcomes %>% filter(restricted_data != "No") %>% nrow()
    ...
    # Process reproducibility by discipline
    {
      ...
      exp_fields$exp_field <- str_to_title(exp_fields$field)
      exp_fields <- exp_fields[c("paper_id","exp_field")]
      
      pr_outcomes_modified_exp_field <- merge(pr_outcomes_modified,exp_fields,by="paper_id",all.x = TRUE,all.y=FALSE)
      
      n_papers_open_and_code_data_exp_econ_polisci <-
        sum(pr_outcomes_modified_exp_field[pr_outcomes_modified_exp_field$exp_field=="Political Science" | pr_outcomes_modified_exp_field$exp_field=="Economics",]$data_shared_type=="available online" &
              pr_outcomes_modified_exp_field[pr_outcomes_modified_exp_field$exp_field=="Political Science" | pr_outcomes_modified_exp_field$exp_field=="Economics",]$code_shared_type=="available online")
      n_papers_exp_econ_polisci <- sum(pr_outcomes_modified_exp_field$exp_field=="Political Science" | pr_outcomes_modified_exp_field$exp_field=="Economics")
      p_papers_open_data_and_code_exp_econ_polisci <- format.text.percent(n_papers_open_and_code_data_exp_econ_polisci,n_papers_exp_econ_polisci)
      
      n_papers_open_data_and_code_non_exp_econ_polisci <-
        sum(pr_outcomes_modified_exp_field[pr_outcomes_modified_exp_field$exp_field!="Political Science" & pr_outcomes_modified_exp_field$exp_field!="Economics",]$data_shared_type=="available online" &
              pr_outcomes_modified_exp_field[pr_outcomes_modified_exp_field$exp_field!="Political Science" & pr_outcomes_modified_exp_field$exp_field!="Economics",]$code_shared_type=="available online")
      n_papers_non_exp_econ_polisci <- sum(pr_outcomes_modified_exp_field$exp_field!="Political Science" | pr_outcomes_modified_exp_field$exp_field!="Economics")
      p_papers_open_data_and_code_non_exp_econ_polisci <- format.text.percent(n_papers_open_data_and_code_non_exp_econ_polisci,n_papers_non_exp_econ_polisci)
      
      p_ratio_papers_open_data_and_code_exp_econ_polisci_vs_other <- bootstrap.clust(data=pr_outcomes_modified_exp_field,FUN=function(x) {
        n_papers_open_data_and_code_exp_econ_polisci <-
          sum(x[x$exp_field=="Political Science" | x$exp_field=="Economics",]$data_shared_type=="available online" &
                x[x$exp_field=="Political Science" | x$exp_field=="Economics",]$code_shared_type=="available online")
        n_papers_exp_econ_polisci <- sum(x$exp_field=="Political Science" | x$exp_field=="Economics")
        n_papers_open_data_and_code_non_exp_econ_polisci <-
          sum(x[x$exp_field!="Political Science" & x$exp_field!="Economics",]$data_shared_type=="available online" &
                x[x$exp_field!="Political Science" & x$exp_field!="Economics",]$code_shared_type=="available online")
        n_papers_non_exp_econ_polisci <- sum(x$exp_field!="Political Science" | x$exp_field!="Economics")
        
        
        (n_papers_open_data_and_code_exp_econ_polisci/n_papers_exp_econ_polisci)/
          (n_papers_open_data_and_code_non_exp_econ_polisci/n_papers_non_exp_econ_polisci)
      },clustervar="paper_id",iters=iters)$formatted.text
    ...
    {
      n_pr_papers_author_gen_data <- nrow(rr_internal_tracking %>%
                                            semi_join(paper_status_tracking %>% filter(RR), by = "paper_id") %>%
                                            filter(str_detect(all_types, "Source Data Reproduction")) %>%
                                            select(paper_id) %>%
                                            bind_rows(
      ...
      data <- paper_status_tracking %>% filter(RR) %>% 
        select(paper_id) %>% 
    ...
    # Outcome reproducibility assessments in comparison with the sample
    {
      ...
      n_papers_OR_source_data_of_all <- rr_internal_tracking %>%
        filter(field != "covid") %>%
        filter(str_detect(type, "Source Data Reproduction")) %>%
        anti_join(
          pr_outcomes %>%
    ...
    {
      n_papers_initial_sample_p1 <- paper_status_tracking %>% filter(p1_delivery) %>% nrow()
      
      n_papers_bushel <- (nrow(paper_status_tracking  %>% filter(bushel)))
    }
    
    # Attrition of reproductions that started but were not completed
    {
      n_papers_repro_completed <- rr_internal_tracking %>%
    ...
    # Supplement: Journal policy analysis
    {
      # Data prep
      {
        ...
        OR_outcomes_journal_policies <- merge(repro_outcomes[repro_outcomes$repro_outcome_overall != "none",],
                                              papers_repro_journal_policies,
                                              by="paper_id") %>% 
          group_by(paper_id) %>% 
          mutate(weight=1/n())
      ...
      # Functions for making table cross-tabs
      {
        ...
        make_cross_tab <- function(df, outcome_var) {
          df$weight <- 1
          make_weighted_cross_tab(df, outcome_var, weight_var="weight")
        }
      ...
      # Table s8
      {
        ...
        table_s8_n <- round(as.numeric(table_s8_n_4_5))
      ...
      # Table s9
      {
        ...
        table_s9_n <- round(as.numeric(table_s9_n_4_5))
      ...
      # Table s12
      {
        ...
        for (row_label in c("1", "0")) {
          ...
          table_s12[row_label, "Total"] <- paste0(
            raw_counts[row_label, "Total"], "\n(", 
            format(round(100*raw_counts[row_label, "Total"]/raw_counts[nrow(raw_counts),ncol(raw_counts)], 1), nsmall = 1), "%)")
        }
      ...
      # Table s10
      {
        ...
        table_s10_n <- format.round(sum(OR_outcomes_journal_policies$weight),0)
