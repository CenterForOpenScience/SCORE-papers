...
tagged_stats <- function(iters = 100,generate_binary_outcomes_data=FALSE){
  ...
  # Supplement stats
  {
    # Tables
    {
        ...
        for (row in 1:nrow(table_s2)){
          for (col in 1:ncol(table_s2)){
            assign(paste0("table_s2_",row,"_",col),
                   as.character(table_s2[row,col][1,1]))
          }
        }
      ...
      # Table S4
      {
        ...
        table_s4 <- rbind(r1,r2,r3,r4,r5,r6,r7,r8)
        table_s4_n <- rbind(r1.n,r2.n,r3.n,r4.n,r5.n,r6.n,r7.n,r8.n)
        table_s4_p <- rbind(r1.p,r2.p,r3.p,r4.p,r5.p,r6.p,r7.p,r8.p)
      ...
      # Table S6
      {
        ...
        table_s6 <- rbind(r1,r2,r3,r4)
        for (row in 1:nrow(table_s6)){
          for (col in 1:ncol(table_s6)){
            assign(paste0("table_s6_",row,"_",col),
                   table_s6[row,col])
            if(row==4){
              assign(paste0("table_s6_median_",row,"_",col),
                     r4.median[col])
              assign(paste0("table_s6_SD_",row,"_",col),
                     r4.SD[col])
              assign(paste0("table_s6_median_formatted_",row,"_",col),
                     r4.median.formatted[col])
              assign(paste0("table_s6_SD_formatted_",row,"_",col),
                     r4.SD.formatted[col])
            }
          }
        }
      ...
      # Table S9
      {
        ...
        table_s9 <- do.call(rbind,lapply(fields.order,function(field_selected) {
    ...
    {
      p_rep_2010_initial <- 
        paste0(format.round(100*table_s4_n$'2010'[1]/table_2_n$Total[1],1),"% [n = ",table_s4_n$'2010'[1],"/",table_s4_n$Total[1],"]")
      p_rep_2010_final <- 
        paste0(format.round(100*table_s4_n$'2010'[8]/table_2_n$Total[8],1),"% [n = ",table_s4_n$'2010'[8],"/",table_s4_n$Total[8],"]")
      
      p_rep_2011_initial <- 
        paste0(format.round(100*table_s4_n$'2011'[1]/table_2_n$Total[1],1),"% [n = ",table_s4_n$'2011'[1],"/",table_s4_n$Total[1],"]")
      p_rep_2011_final <- 
        paste0(format.round(100*table_s4_n$'2011'[8]/table_2_n$Total[8],1),"% [n = ",table_s4_n$'2011'[8],"/",table_s4_n$Total[8],"]")
      
      p_rep_2015_initial <- 
        paste0(format.round(100*table_s4_n$'2015'[1]/table_2_n$Total[1],1),"% [n = ",table_s4_n$'2015'[1],"/",table_s4_n$Total[1],"]")
      p_rep_2015_final <- 
        paste0(format.round(100*table_s4_n$'2015'[8]/table_2_n$Total[8],1),"% [n = ",table_s4_n$'2015'[8],"/",table_s4_n$Total[8],"]")
      
      p_max_expectation_repli_completed_except_2015 <- paste0(format.round(100*max(
        abs(table_s4_p$"2009"[6]-.1),
        abs(table_s4_p$"2010"[6]-.1),
        abs(table_s4_p$"2011"[6]-.1),
        abs(table_s4_p$"2012"[6]-.1),
        abs(table_s4_p$"2013"[6]-.1),
        abs(table_s4_p$"2014"[6]-.1),
        abs(table_s4_p$"2016"[6]-.1),
        abs(table_s4_p$"2017"[6]-.1),
        abs(table_s4_p$"2018"[6]-.1)
      ),1),"%")
      p_repli_completed_2015 <- paste0(format.round(100*
                                                      abs(table_s4_p$"2015"[6]),1),"%")
    }
...
figures <- function(iters = 100,generate_binary_outcomes_data=FALSE){
  ...
  # Supplement figures
  {
    ...
    # Figure S2
    {
      ...
      figure_s2 <- plot_grid(figure_s2_p1,figure_s2_p2,ncol=1)
    }
    
    # Figure S3
    {
      figure_s3 <- repli_outcomes_orig %>% 
        filter(!is_covid & repli_version_of_record) %>% 
        left_join(paper_metadata %>% select(paper_id, field = COS_pub_category), by = "paper_id") %>% 
        left_join(
          orig_dataset %>% 
    ...
    # Figure S10
    {
      ...
      # Chart generation
      {
        ...
        figure_s10 <- plot_grid(top,bottom,ncol=1,rel_heights = c(5,1),align = "v")
      }
    ...
    # Figure S11
    {
      figure_s11 <- repli_outcomes_orig %>% 
        filter(!is_covid & repli_version_of_record) %>% 
    ...
    # Figure S12
    {
      ...
      {
        #ggplot(binary.proportions, aes(x=binary.var,y=PointEst)) + 
        figure_s12 <- ggplot(binary.proportions, aes(x=reorder(binary.var, -p_passed_papers),y=p_passed_papers)) + 
          geom_bar(stat="identity",fill="grey90") +
          geom_text(aes(x=num,y=0.02,label=n.text),hjust=0,size=3)+
          funkyheatmap::geom_rounded_rect(
            aes(xmin=num-.45,xmax=num+.45,ymin = p_passed_CI_LB_papers, ymax = p_passed_CI_UB_papers),
