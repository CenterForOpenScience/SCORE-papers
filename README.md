...
Next, run `usethis::edit_r_environ()` in the console to open your .Renviron file. Enter 'google_oauth_email="youremail\@cos.io"' as a new line in the file, making sure to replace "youremail\@cos.io" with your actual email within quotations. This will allow Google to silently authenticate without needing to change any code. If you have not used the packages `googledrive` or `googlesheets4` with the email denoted in the .Renviron file, run `googledrive::drive_auth()` and `googlesheets4::gs4_auth()` in the console to complete a one-time authentication through your browser.
